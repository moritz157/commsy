...
var soapClient;
exports.init = function (wsdl_location, apiEndpoint, callback){
  soap.createClient(wsdl_location, function(err, client){
...
exports.getTypes = function(){
  var types = [];
  var text = "";
  for(var property in soapClient.describe().CommSyService.CommSyPort){
    //for(var inputProperty in soapClient.describe().CommSyService.CommSyPort[property].input){
    //text+=JSON.stringify(soapClient.describe().CommSyService.CommSyPort[property]);
    //console.log(soapClient.describe().CommSyService.CommSyPort[property]);
    for(var inputProperty in soapClient.describe().CommSyService.CommSyPort[property].input){
      text=soapClient.describe().CommSyService.CommSyPort[property].input[inputProperty];
...
/*exports.auth = function(user_id, password, portal_id, auth_source_id, callback){
  ...
  soapClient.authenticate({"user_id":user_id, "password":password, "portal_id":portal_id, "auth_source_id":auth_source_id}, function(err, result){
    if(err){
      ...
      console.log(result);
      var newResult={'$value':result.return.$value};
    }
...
exports.generate = function(){
  var fullGenerated = "";
  var fullGeneratedApp = "";
  console.log(soapClient.describe().CommSyService.CommSyPort.authenticate);
  for(var property in soapClient.describe().CommSyService.CommSyPort){
    var generated = "exports."+property+" = function(";
    var generatedApp = "app.post('/"+property+"', function(req, res){\n\tcommsy."+property+"(";
    var appDoc = "/**\n * @api {post} /"+property + " " + property + "\n * @apiGroup Main";
    var counter=0;
    if(soapClient.describe().CommSyService.CommSyPort[property].input){
      for(var inputProperty in soapClient.describe().CommSyService.CommSyPort[property].input){
        if(counter>0){
          generated+=", ";
          generatedApp+=", ";
        }
        generated+=inputProperty;
        generatedApp+="req.body."+inputProperty;
        appDoc+="\n * @apiParam {";
        switch(soapClient.describe().CommSyService.CommSyPort[property].input[inputProperty]){
          case "xsd:integer":
            appDoc+="Number} ";
            break;
          case "xsd:string":
    ...
    appDoc+="\n **/\n";
    generatedApp+="function(err, result){\n\tif(err){\n\t\tconsole.log('[ERROR] '+ err);\n\t\tres.status(400);\n\t\tres.send({'Error':err});\n\t}else{\n\t\tconsole.log(result);\n\t\t\tres.send(result);\n\t\t}\n\t});\n});\n";
    counter=0;
    for(var inputProperty in soapClient.describe().CommSyService.CommSyPort[property].input){
      if(counter>0){
        generated+=", ";
      }
      generated+='"'+inputProperty+'":'+inputProperty;
      counter++;
    }

    if(property.indexOf("AsJson")>-1){
      generated+="}, function(err, result){\n\t\tif(err){\n\t\t\tconsole.log(err)\n\t\t}else{\n\t\t\tresult.return.$value=JSON.parse(result.return.$value)}\n\t\tcallback(err, result)\n\t})\n}\n";
    }else{
      generated+="}, function(err, result){\n\t\tif(err){\n\t\t\tconsole.log(err)\n\t\t}\n\t\tcallback(err, result)\n\t})\n}\n";
    }
    console.log(generatedApp);
    fullGenerated+=generated;
    fullGeneratedApp+=appDoc+generatedApp;
  }
  return {"module":fullGenerated,"rest":fullGeneratedApp};
}
